...
### Задание 3. Наследование и инкапсуляция
...
1. Создайте базовый класс MusicFile с методом play(), который выводит "Воспроизведение аудио файла...".
2. Реализуйте три класса-наследника:
* MP3File – должен переопределять play() и выводить "Воспроизведение MP3 файла"
* WAVFile – должен переопределять play() и выводить "Воспроизведение WAV файла"
* FLACFile – должен переопределять play() и выводить "Воспроизведение FLAC файла"
3. Напишите функцию play_music(files: list), которая принимает список файлов и вызывает у каждого из них метод play().
4. Протестируйте программу, создав список из экземпляров MP3File, WAVFile и FLACFile, затем передайте его в play_music().
```
Воспроизведение MP3 файла
Воспроизведение WAV файла
Воспроизведение FLAC файла
```

### Задание 5. Абстракция

Создайте систему для управления транспортными средствами с использованием абстракции.

1. Создайте абстрактный класс Vehicle, который содержит:
* Абстрактный метод move(), который будет определять, как передвигается транспорт.
* Абстрактный метод fuel_type(), который возвращает тип топлива.
2. Реализуйте два подкласса:
* Car – должен переопределять move() и fuel_type(), например:
```
move() -> "Автомобиль едет по дороге"
fuel_type() -> "Бензин"
```
* Boat – должен переопределять move() и fuel_type(), например:
```
move() -> "Лодка плывёт по воде"
fuel_type() -> "Дизель"
```
3. Напишите функцию test_transport(vehicles: list), которая принимает список объектов транспорта и вызывает у каждого методы move() и fuel_type().
4. Протестируйте код, создав объекты Car и Boat, затем передайте их в test_transport().
```
