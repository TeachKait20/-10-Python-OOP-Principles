...
damage: <dmg>
```
**2.** Создайте класс Archer (Лучник), который будет наследоваться от класса `Personage`. Каждый класс должен описывать конкретный тип героя, например:
Archer - будет иметь все атрибуты класса Personage и дополнительно ещё один свой - weapon (weapon). <br>
